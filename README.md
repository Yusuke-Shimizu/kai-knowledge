...
> **⚠️ ここに入るのは完全架空案件 KAI のデータだけ。実案件のデータは入れない。**
> 実在の顧客名・社内名・個人名は書かない。実案件の正本は**案件ごとに別リポジトリ**を立てる。

**public。** 架空案件なので公開して差し支えなく、そのぶん GitHub Free でも
**ruleset で `main` を保護できる**（private だと Pro が要る）。Actions の実行時間も食わない。
「エージェントが書けるのは提案まで、正本を変えるのは人のマージだけ」を**設定で担保する**ために
この可視性を選んでいる。
